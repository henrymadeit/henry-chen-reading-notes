...
__name(makeAdminToken, "makeAdminToken");
async function isAdmin(context) {
  const cookie = context.request.headers.get("Cookie") ?? "";
  const adminCookie = cookie.split(";").map((part) => part.trim()).find((part) => part.startsWith("henry_admin="));
...
var onRequestGet = /* @__PURE__ */ __name(async (context) => {
  ...
  const { results } = await context.env.henryreads_comments.prepare(`
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (name.length > 40 || message.length > 1e3) {
      return Response.json(
        { error: "\u59D3\u540D\u6216\u7559\u8A00\u592A\u9577" },
        { status: 400 }
      );
    }
    const result = await context.env.henryreads_comments.prepare(`
...
var onRequestPatch = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    if (action === "reply") {
      ...
      const result2 = await context.env.henryreads_comments.prepare(`
          UPDATE comments
          SET reply = ?,
      ...
      return Response.json({
        success: true,
        reply: result2.reply,
        reply_updated_at: result2.reply_updated_at
      });
    }
    if (action === "reply-like" || action === "reply-unlike") {
      ...
      return Response.json({
        success: true,
        reply_likes: result2.reply_likes
      });
    }
...
var onRequestGet2 = /* @__PURE__ */ __name(async (context) => {
  ...
  const row = await context.env.henryreads_likes.prepare("SELECT count FROM likes WHERE slug = ?").bind(slug).first();
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
  ...
  await context.env.henryreads_likes.prepare(`
...
// api/admin-login.ts
async function makeAdminToken2(password) {
  const data = new TextEncoder().encode(`henryreads-admin:${password}`);
  const hash = await crypto.subtle.digest("SHA-256", data);
  return Array.from(new Uint8Array(hash)).map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
__name(makeAdminToken2, "makeAdminToken");
var onRequestPost3 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const token = await makeAdminToken2(context.env.ADMIN_PASSWORD);
    return Response.json(
      { success: true },
      {
        headers: {
          "Set-Cookie": `henry_admin=${token}; Path=/; HttpOnly; Secure; SameSite=Strict`
        }
      }
    );
  } catch {
...
// api/admin-status.ts
async function makeAdminToken3(password) {
  const data = new TextEncoder().encode(`henryreads-admin:${password}`);
  const hash = await crypto.subtle.digest("SHA-256", data);
  return Array.from(new Uint8Array(hash)).map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
__name(makeAdminToken3, "makeAdminToken");
var onRequestGet3 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const token = adminCookie.substring("henry_admin=".length);
    const expectedToken = await makeAdminToken3(context.env.ADMIN_PASSWORD);
    return Response.json({
...
var onRequestPost4 = /* @__PURE__ */ __name(async () => {
  return new Response(
    JSON.stringify({ success: true }),
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "Set-Cookie": "henry_admin=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0"
      }
    }
  );
}, "onRequestPost");
var onRequestGet4 = /* @__PURE__ */ __name(async () => {
  return Response.json({
    ok: true,
    message: "admin-logout function exists"
  });
}, "onRequestGet");
